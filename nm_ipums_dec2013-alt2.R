...
run.prog.epsdist.alt <- function(niter=21){
  s2kyy <- mkyy <- pkyy <- array(0,dim=c(length(tau.old),length(ybar.sample),length(ybar.sample)),dimnames=c("cmp","yi","ybar"))
  for(iter in 1:niter){
    cat("iteration: ",iter,"\n")
## y.sample <- ybar               
...
## plot(y.sample,pky[5,])
    ...
    s2ky.p <- mky.p <- pky.p <- matrix(0,nrow=length(tau.old),ncol=length(ybar))
    ### HERE CHANGES MADE ###
    ai <-  1:length(ybardata$area)
    area <- ybardata$area
#    for(a in unique(ydata$area)){
    for(a in ai){
      yvals <- ydata$y[ydata$area==area[a]]
      ybar.a <- ybardata$ybar[ybardata$area==area[a]]
      for(k in 1:length(tau.old)){
        pky.p[k,a] <- sum(bicubic(y.sample,ybar.sample,pkyy[k,1:n.y.sample,1:n.ybar.sample],yvals,rep(ybar.a,length(yvals)))$z) 
        mky.p[k,a] <- sum(bicubic(y.sample,ybar.sample,mkyy[k,1:n.y.sample,1:n.ybar.sample],yvals,rep(ybar.a,length(yvals)))$z) 
        s2ky.p[k,a] <- sum(bicubic(y.sample,ybar.sample,s2kyy[k,1:n.y.sample,1:n.ybar.sample],yvals,rep(ybar.a,length(yvals)))$z) 
...
## Hier ging iets fout: ybardata$u bestaat niet.
...
rm(list=setdiff(ls(),c(all.functions,"all.functions","mu.new","ppi.new","sig.new","sig2.new","v.eps","v.u","ybardata","ydata","sigma2.eps","sigma2.v","reg.beta","beta.gls","sur.data","sur.data.u","run.prog.epsdist.alt")))

################################### EPS DISTRIBUTION #####################
###### eps distribution ####

save.image(file="nmdec2013-b.RData")
load(file="nmdec2013-b.RData")
library(akima)
...
reset.pars.eps()
(outcomes <- run.prog.epsdist.alt(13)) ## again, use more iterations if you want
outcomes13 <- outcomes
########## hierrrrr  ######
...
####### temporary
lam.ini <- lam.old <- as.numeric(outcomes$lam.new)
nu.ini <- nu.old <- as.numeric(outcomes$nu.new)
tau.ini <- tau.old <- as.numeric(outcomes$tau.new)
tau2.ini <- tau2.old <- as.numeric(outcomes$tau2.new)
...
ydata.ori <- ydata$y
set.seed(20131213)
(replace.values <- rnorm(s=sd(ydata$y),n=3))
## > (replace.values <- rnorm(s=sd(ydata$y),n=3))
## [1] 0.2420690 0.4159338 0.8233661

ydata$y[ydata$y < -7] <- replace.values 
head(sort(ydata$y))
reset.pars.eps()
(outcomes15 <- run.prog.epsdist.alt(15))

ydata$y <-  ydata.ori ## terugzetten originele data
####### temporary
