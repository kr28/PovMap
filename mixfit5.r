...
setwd("~/Dropbox/Shared with Chris")
...
fv.blok.restricted <- function(niter) {
  ...
  for(k in 1:niter) {
    ...
    progr.llh <<- rbind(progr.llh,c(p,mu,sig,llh1,0)) # note the double arrow: this changes global variable progr.llh
  }
  list(p=p,mu=mu,sig=sig,llh1=llh1) 
}


progr.llh <- c(p,mu,sig,0,0) # collect steps in EM algorithm in prog.llh
p.c <- paste("p",1:g,sep="")
mu.c <- paste("mu",1:g,sep="")
sig.c <- paste("sig",1:g,sep="")
names(progr.llh) <- c(p.c,mu.c,sig.c,"llh1","llh2")
...
feps.blok.restricted <- function(niter) {
  ...
  list(p=p,mu=mu,sig=sig,llh1=llh1,llh2=0)
}
