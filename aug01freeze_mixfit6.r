...
#
data <- read.dta("phi2009test3.dta")
...
X<-as.matrix(subset(data,select=c(hh_logsize,hd_female,hd_logage,hd_married,hd_unemp,hd_olf,hd_edu4,hd_edu5,hd_edu6,min1_emp,min1_edu56,pch_5,pch_516,peld_60,dw_basic,urb_logsize,urb_hd_fem,urb_hd_unemp,urb_hd_olf,urb_hd_edu5,urb_hd_edu6,urb_min_emp,urb_min_edu56,urb_pch5,urb_pch516,urb_peld60,urb_dw_basic)))
X.temp<-as.matrix(subset(data,select=c(urban,hh_logsize,hd_female,hd_logage,hd_married,hd_unemp,hd_olf,hd_edu4,hd_edu5,hd_edu6,min1_emp,min1_edu56,pch_5,pch_516,peld_60,dw_basic,urb_logsize,urb_hd_fem,urb_hd_unemp,urb_hd_olf,urb_hd_edu5,urb_hd_edu6,urb_min_emp,urb_min_edu56,urb_pch5,urb_pch516,urb_peld60,urb_dw_basic)))
X.ols<-matrix(0,dim(X.temp)[1],dim(X.temp)[2]+1)
X.ols[,2:dim(X.ols)[2]]<-X.temp
...
# H3 variance of epsilon: FELM
v.eps.bgy1 <- summary(fereg_bgy)$rse^2    # v.eps.bgy<-sqrt((t(eps_felm) %*% eps_felm)/(38369-26-3015))

# Get var(eps)/k_a
ka <- as.numeric(table(data$id_bgy))
#v.eps <- v.eps.bgy1 / ka

# H3 variance of epsilon: within estimator
y <- data$lny
Dka <- diag(ka)
ybar <- aggregate(data$lny,by=list(data$id_bgy),FUN=mean)$x
a.X <- aggregate(X,by=list(data$id_bgy),FUN=mean)
Xbar <- as.matrix(a.X[,2:dim(a.X)[2]])
XTX <- (t(X)%*%X)-(t(Xbar)%*%Dka%*%Xbar)
XTy <- (t(X)%*%y)-(t(Xbar)%*%Dka%*%ybar)
beta.within <- ginv(XTX)%*%XTy
eps.within <- (y-rep(ybar,ka))-(X%*%beta.within-rep(Xbar%*%beta.within,ka))
v.eps.bgy2 <- sum(eps.within*eps.within)/(n_hlds-(d-1)-n_area)

v.eps <- v.eps.bgy2 / ka

# H3 variance of v: Corollary 3
a.X.ols <- aggregate(X.ols,by=list(data$id_bgy),FUN=mean)
UX.ols <- as.matrix(a.X.ols[,2:dim(a.X.ols)[2]])
UX.ols <- Dka%*%UX.ols
trupxu <- tr(ginv(t(X.ols)%*%X.ols)%*%t(UX.ols)%*%UX.ols)
v.v.bgy1A <- (n_hlds-d.ols)*((sum(eps*eps)/(n_hlds-d.ols))-v.eps.bgy2)/(n_hlds-trupxu)
v.v.bgy1B <- var(eps)-v.eps.bgy1

# H3 variance of v: Simple moment condition
v.v.bgy2A <- (n_hlds-d)*((sum(eps*eps)/(n_hlds-d))-v.eps.bgy2)/(n_hlds-trupxu)
v.v.bgy2B <- var(eps)-v.eps.bgy2
